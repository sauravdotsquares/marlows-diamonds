...
<footer class="footer-main">
                ...
                <div class="fcontent-column icon-payment">
                    ...
                    <div class="footer-social">
                        @if ($header_settings['facebook'] != '')
                            <a href="{{ $header_settings['facebook'] }}" target="_blank" rel="follow"><i
                                    class="fa fa-facebook" aria-hidden="true" aria-label="facebook"></i></a>
                        @endif
                        @if ($header_settings['twitter'] != '')
                            <a href="{{ $header_settings['twitter'] }}" target="_blank" rel="follow"><i
                                    class="fa fa-twitter" aria-hidden="true" aria-label="twitter"></i></a>
                        @endif
                        @if ($header_settings['instagram'] != '')
                            <a href="{{ $header_settings['instagram'] }}" target="_blank" rel="follow"><i
                                    class="fa fa-instagram" aria-hidden="true"aria-label="instagram"></i></a>
                        @endif
                        @if ($header_settings['pinterest'] != '')
                            <a href="{{ $header_settings['pinterest'] }}" target="_blank" rel="follow"><i
                                    class="fa fa-pinterest" aria-hidden="true"aria-label="pinterest"></i></a>
                        @endif
                        @if ($header_settings['youtube'] != '')
                            <a href="{{ $header_settings['youtube'] }}" target="_blank" rel="follow"><i
                                    class="fa fa-youtube" aria-hidden="true"aria-label="youtube"></i></a>
                        @endif
                        @if ($header_settings['linkedin'] != '')
                            <a href="{{ $header_settings['linkedin'] }}" target="_blank" rel="follow"><i
                                    class="fa fa-linkedin" aria-hidden="true"aria-label="linkedin"></i></a>
                        @endif
                    </div>
                    <div class="fcontent-column disclaimer-content">
                        <b>Birmingham Store:</b> 46-47 Warstone Lane Hockley, Birmingham B18 6JJ.
                        <b>Email: </b><a style="color:#fff; text-decoration: none;"
                            href="mailto:hello@marlows-diamonds.co.uk">hello@marlows-diamonds.co.uk</a>
    ...
    <div class="bottom-eye-icon">
        ...
        <span class="tooltiptext">10% Price beat service through Whatsapp. Send us a link of what you have seen
            elsewhere to get a minimum of 10% off competitor's price. Only through whatsapp contact on any lab grown
            product</span>
        {{-- </a> --}}
    </div>
    {{-- eye icon --}}
