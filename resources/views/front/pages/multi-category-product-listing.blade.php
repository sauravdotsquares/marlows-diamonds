...
<div class="category-listing-wrap" ng-controller="ProductController" ng-cloak>
    <div class="container"  >
        {{-- ng-init="productCatFilters('{{$cat1}}','{{$cat2}}','{{$cat3}}')" --}}
                        ...
                        @foreach(session('cart') as $id => $details)
                            ...
                            <div class="side-cart-item">
                                ...
                                <div class="side-cart-quantity">
                                    {{ $details['quantity'] }} ×
                                    <span class="side-cart-amount">{{config("constants.MY_CURRENCY_SYMBOL")}}{{ number_format($details['price'],2) }}</span>
                                </div>
                                <div class="side-cart-total">
                                    <strong>Subtotal: </strong> {{config("constants.MY_CURRENCY_SYMBOL")}}{{ number_format($details['price'] * $details['quantity'],2) }} 
                                </div>
...
$(document).ready(function(){
    ...
    <script>
            ...
            $(document).on('scroll',function(){
                if(triggerScrollEvent){
                    if($(".ajax-load").isInViewport()){
                        ...
                        $(".more-records").trigger('click');
                    }
                }
            });
        });

        /** Start video on mouse hover */
        ...
        /** on click of more records button */
        $(".more-records").on('click', function() {
            getData($(this).attr('data-pageNo'));
        });
        ...
        function getData(pageNo){
            $.ajax({
                ...
                if(data.status){
                    $(".more-records").attr('data-pageNo', data.nextPage);
        ...
        /** Play videos on hover on mobile */
