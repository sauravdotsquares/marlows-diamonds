<li class="cc_payment_methods paypal_payment paypalpaymentgateway_wrap">
    <div class="paypal_button">
        <input type="radio" name="payment_type" checked required="required" value="paypal" autocomplete="off">
        <label class="paypal_label">
            Paypal
            <a class="what-paypal" href="https://www.paypal.com/gb/webapps/mpp/paypal-popup"
                onclick="javascript:window.open('https://www.paypal.com/gb/webapps/mpp/paypal-popup','WIPaypal','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700'); return false;">What
                is PayPal?</a>
        </label>
        <i class="diamond-icon paypent-checkout"></i>
    </div>
    <div class="payment-box-main-drop paypal-pay-box" style="display: none;">
        Pay via PayPal; you can pay with your credit card if you don’t have a PayPal
        account.
    </div>
</li>
