...
async function onClick() {
    const inputValue = document.getElementById('final_price').value;
...
function updateOrderStatusApple(paymentData) {
   const paymentMethod = paymentData.payment.token.paymentMethod.displayName; // e.g. "Visa••••0156"
   
   const token = paymentData.payment.token.transactionIdentifier; 
   
   const billingAddress = paymentData.payment.billingContact;
   const countryCode = billingAddress.countryCode; // e.g. "IN"

   var tokenOrdIdUpdated = $('#tokenOrdId').val();

  // Prepare data for updating the order status
  const orderUpdateData = {
      paymentMethod: paymentMethod,
      token: token,
      billingAddress: billingAddress,
      countryCode: countryCode,
      tokenOrdIdUp: tokenOrdIdUpdated
      // Additional fields as necessary
  };
  

  // Send data to the backend (Laravel) to update the order status
  ...
  if (csrfToken) {
      ...
      fetch('/update-order-status', {
          ...
          headers: {
              'Content-Type': 'application/json',
              'X-CSRF-TOKEN': csrfToken.getAttribute('content') // CSRF token
          },
