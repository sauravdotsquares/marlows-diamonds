...
class WishlistController extends Controller
{
    ...
    public function addToWishlist(Request $request)
    {
        $product = Products::with(['getProductImages', 'getProductVariation'])
        ...
        // Toggle remove if product already exists
        if (isset($wishlist[$product->id])) {
            $this->removeWishlist(new Request(['id' => $product->id]));
        ...
        $wishlist[$product->id] = [
            ...
            "titleSlug"    => $product->slug,
            "price"        => $request->price,
            "added_date"   => now()->format('M d, Y'),
            "stock_status" => "In stock",
            "image"        => optional($product->getProductImages)->image_url
        ];
