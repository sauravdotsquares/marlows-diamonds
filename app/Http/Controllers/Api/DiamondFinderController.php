...
class DiamondFinderController
{
    public function diamondSearch(Request $request)
    {
        ...
        $data = [
            ...
            'clarityTo' => $clarityTo,
            'clarity' => $clarity,
            'caratFrom' => $request->carat_min,
            'caratTo' => $request->carat_max,
            'gradeFrom' => $gradeFrom,
            'gradeTo' => $gradeTo,
            'grade' => $grade,
        ...
        $hkData['data'] = array_map([$this, "amountChange"], $hkData['data']);

        if (empty($hkData['data']) || (!empty($hkData['data']) && $hkData['total'] < 50)) {
            $hkData['to'] = 5;
            $hkData['last_page'] = 10;
            $hkData['total'] = 100;
        }

        // Fetch RapNet Data (safe handling)
        $rapnetRecords = [];
        try {
            $rapnetData = getRapnetApiRecordsDiamondSearch($data, $hkData['current_page']);

            if (!empty($rapnetData) && !is_string($rapnetData)) {
                ...
                \Log::warning('RapNet returned empty or error string: ' . print_r($rapnetData, true));
            }
        } catch (\Exception $e) {
            \Log::error('RapNet API failed: ' . $e->getMessage());
            // continue with only HK data
        }
        ...
        usort($hkData['data'], function ($a, $b) {
            return $a['Amount'] - $b['Amount']; // ascending sort
        });
    ...
    {
        $marginAPIPercentage = MarginApiRange::where('api_type', 'harikrishna')
            ->whereRaw('"' . $num['Amount'] . '" between `from_price` and `to_price`')
            ->where('status', 1)
            ->first();

        $num['oldAmount'] = $num['Amount'];
        if (isset($num['Amount']) && $marginAPIPercentage) {
